...
# avg_data is a utility function to get the PM 2.5 values of a month.
# It takes year as a parameter.


def avg_data(x):
    temp_i = 0
    average = []
    path_name = 'Data/AQI/aqi' + str(x) + '.csv'
    # Chunk size of 24 denotes 24 hours in each day
    for rows in pd.read_csv(path_name, chunksize=24):
        add_var = 0
        avg = 0.0
        data = []
